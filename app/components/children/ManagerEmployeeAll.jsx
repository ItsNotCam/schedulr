...
const ManagerEmployeeAll = () => {
  ...
  const getEmployees = () => {
    const get = async () => {
      const response = await helpers.getAllEmployees();
      if (response !== state.allEmployees) {
        setState({ allEmployees: response.data });
        activeButtons();
      }
    };
    get();
  ...
  const handleAddForm = (event) => {
    ...
    helpers
      ...
      .then(function (response) {
        state.emp_id = response.data._id;

        helpers
          .addEmpSchedule(state.emp_id, state.firstName, state.lastName)
          .then(function (response) {
            clearStates();
          });
      });
  ...
  const handleUpdateForm = (event) => {
    ...
    helpers
      .updateEmpName(state.emp_id, state.firstName, state.lastName)
      .then(function (response) {
        clearStates();
      });
  ...
  const handleRemoveForm = (event) => {
    event.preventDefault();
    helpers.removeEmployee(state.selectedEmployee).then(function (response) {});
    helpers.removeEmpSchedule(state.emp_id).then(function (response) {
      clearStates();
    });
  ...
  const clickEmployee = (event) => {
    setState({ selectedEmployee: event.target.id });
          ...
          <tbody>
            <tr>
              <td id="newEmployee" onClick={newEmployee}>
                <strong>
                  New Employee<i className="material-icons right">add</i>
                </strong>
              </td>
            </tr>
            {state.allEmployees.map(function (ManagerEmployeeAll, i) {
              return (
                <tr key={i}>
                  <td onClick={clickEmployee} id={state.allEmployees[i]._id}>
